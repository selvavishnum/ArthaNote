...
class AudioParams {
  // Noise Reduction (Pass 1 — Wiener)
  final double nrStrength;   // 0–100
  final double nrAlpha;      // 0–99 (smoothing)
  final double nrFloor;      // 1–12 (noise floor multiplier)

  // Hard Spectral Gate (Pass 2)
  final double gateThreshold; // 0–100
  final double gateRatio;     // 0.5–5.0 ×noise floor

  // VAD Gate (Pass 3)
  final double vadSensitivity; // 1–10 ×noise floor
  final int    vadHoldMs;      // 20–300 ms
...
class AudioData {
  ...
  AudioData({required this.samples, required this.sampleRate})
      : duration = Duration(
